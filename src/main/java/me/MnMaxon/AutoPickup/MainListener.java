...
public class MainListener implements Listener {
    ...
    public void onClick(PlayerInteractEvent e) {
        try {
            if (Config.usingAutoSell && e.getAction().name().toLowerCase().contains("right")
                    && e.getPlayer().isSneaking() && e.getItem().getType().name().toLowerCase().contains("pickaxe"))
    ...
    public void onKill(EntityDeathEvent e) {
        ...
        if (killer == null || e.getEntity() instanceof Player
                || AutoPickupPlugin.getBlockedWorlds().contains(killer.getWorld()))
            return;
        if (Config.autoMob) {
    ...
    public void onFish(PlayerFishEvent e) {
        if (!Config.getBlockedWorlds().contains(e.getPlayer().getWorld()) && Config.autoMob && e.getCaught() != null
    ...
    public void onBlockBreak(BlockBreakEvent e) {
        ...
        if (AutoPickupPlugin.getBlockedWorlds().contains(e.getPlayer().getWorld()))
            return;
        String name = e.getPlayer().getName();
        SuperLoc.add(e.getBlock().getLocation(), e.getPlayer(), AutoPickupPlugin.autoPickup.contains(name),
                AutoPickupPlugin.autoSmelt.contains(name), AutoPickupPlugin.autoBlock.contains(name), inhand);
        if (Config.infinityPick && e.getPlayer().hasPermission("AutoPickup.infinity")
