...
public class Common
{
    ...
    public static void openGui(Player p)
    {
        ...
        conts[0] = AutoPickupPlugin.easyItem(ChatColor.GREEN + "AutoPickup", Material.HOPPER, 1, 0, ChatColor.GRAY + "Sends mined blocks", ChatColor.GRAY + "straight to your inventory"); 
        conts[1] = AutoPickupPlugin.easyItem(ChatColor.GREEN + "AutoBlock", Material.IRON_BLOCK, 1, 0, ChatColor.GRAY + "Turns ingots into blocks"); 
        conts[2] = AutoPickupPlugin.easyItem(ChatColor.GREEN + "AutoSmelt", Material.FURNACE, 1, 0, ChatColor.GRAY + "Smelts all mined ores"); 
        if (Config.usingQuickSell)
        {
            conts[3] = AutoPickupPlugin.easyItem(ChatColor.GREEN + "AutoSell", Material.GOLD_INGOT, 1, 0, ChatColor.GRAY + "Sells any possible", ChatColor.GRAY + "mined blocks"); 
        }
        ...
        //TODO: make these real if statements   
        conts[9] = (p.hasPermission("AutoPickup.Toggle"))?AutoPickupPlugin.easyItem(autoPickupName, Material.INK_SACK, 1, apDur, ChatColor.GRAY + "Click to Toggle"):AutoPickupPlugin.easyItem(autoPickupName, Material.INK_SACK, 1, apDur); 
        conts[10] = (p.hasPermission("AutoBlock.Toggle"))?AutoPickupPlugin.easyItem(autoBlockName, Material.INK_SACK, 1, abDur, ChatColor.GRAY + "Click to Toggle"):AutoPickupPlugin.easyItem(autoBlockName, Material.INK_SACK, 1, abDur); 
        conts[11] = (p.hasPermission("AutoSmelt.Toggle"))?AutoPickupPlugin.easyItem(autoSmeltName, Material.INK_SACK, 1, asDur, ChatColor.GRAY + "Click to Toggle"):AutoPickupPlugin.easyItem(autoSmeltName, Material.INK_SACK, 1, asDur); 
        if (Config.usingQuickSell)
        {
            conts[12] = (p.hasPermission("AutoSell.Toggle"))?AutoPickupPlugin.easyItem(autoSellName, Material.INK_SACK, 1, aSellDur, ChatColor.GRAY + "Click to Toggle"):AutoPickupPlugin.easyItem(autoSellName, Material.INK_SACK, 1, aSellDur); 
        }

        ItemStack locked = AutoPickupPlugin.easyItem(ChatColor.RED + "LOCKED", Material.STAINED_GLASS_PANE, 1, 14); 
        ItemStack empty = AutoPickupPlugin.easyItem(null, Material.STAINED_GLASS_PANE, 1, 7); 

        //TODO: make these real if statements   
        conts[7] = (p.hasPermission("AutoBlock.Command"))?AutoPickupPlugin.easyItem(ChatColor.GREEN + "Block Your Items", Material.IRON_INGOT, 1, 0):locked; 
        conts[8] = (p.hasPermission("AutoSmelt.Command"))?AutoPickupPlugin.easyItem(ChatColor.GREEN + "Smelt Your Items", Material.COAL, 1, 0):locked; 
        if (Config.extraInfo)
        {
            conts[16] =AutoPickupPlugin.easyItem(ChatColor.RED + "Suggestions/Found Bugs", Material.LAVA_BUCKET, 1, 0, ChatColor.GRAY + "Contact " + ChatColor.GOLD + "MnMaxon" + ChatColor.GRAY + " on ", ChatColor.GRAY + "Spigot or Bukkit", ChatColor.GRAY + "Or, by email:", ChatColor.GRAY + "masontg777@aol.com"); 
        }
        conts[17] =AutoPickupPlugin.easyItem(ChatColor.RED + "Close", Material.ARROW, 1, 0); 
